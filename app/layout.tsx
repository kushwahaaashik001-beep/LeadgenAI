...
export default function RootLayout({
  ...
  return (
    <html lang="en" className="dark" suppressHydrationWarning>   {/* ⭐ Key fix */}
      <head>
      ...
      <body className={`${inter.className} bg-gray-950 text-gray-100 antialiased`} suppressHydrationWarning>   {/* ⭐ Key fix */}
        <Scripts />
        <UserProvider>
                ...
                <div className="h-full p-4">
                  <div className="mb-8">
                    ...
                    <nav className="space-y-1">
                      ...
                      <SidebarLink href="/leads" icon="🎯" badge={12}>Leads</SidebarLink>
                      <SidebarLink href="/ai-pitch" icon="🤖" proOnly>AI Pitch</SidebarLink>
              ...
              <div className="max-w-7xl mx-auto px-4 py-6">
                <div className="flex flex-col md:flex-row items-center justify-between">
                  <div className="mb-4 md:mb-0">
                    ...
                    <p className="text-xs text-gray-500 mt-1">Real-time AI-powered lead generation</p>
                  </div>
                  <div className="flex items-center space-x-6">
                    ...
                    <a href="/support" className="text-xs text-gray-400 hover:text-white transition-colors">Support</a>
                    <div className="text-xs text-gray-500">© {new Date().getFullYear()} LeadGenAI. All rights reserved.</div>
                  </div>
                </div>
                <div className="mt-6 pt-6 border-t border-gray-800 text-center">
                  <p className="text-xs text-gray-500">Powered by Groq AI • Supabase • Next.js • Vercel</p>
...
function SidebarLink({ href, icon, children, badge, proOnly = false }: { href: string; icon: string; children: React.ReactNode; badge?: number; proOnly?: boolean }) {
  return (
    <a href={href} className="flex items-center justify-between px-3 py-2 rounded-lg hover:bg-gray-800 transition-colors group">
      <div className="flex items-center space-x-3">
        <span className="text-lg">{icon}</span>
        <span className="text-sm text-gray-300 group-hover:text-white">{children}</span>
      </div>
      <div className="flex items-center space-x-2">
        {proOnly && <span className="text-[10px] bg-gradient-to-r from-purple-600 to-pink-600 text-white px-2 py-0.5 rounded-full">PRO</span>}
        {badge && <span className="text-xs bg-blue-500 text-white w-6 h-6 rounded-full flex items-center justify-center">{badge}</span>}
      </div>
    </a>
  );
}
